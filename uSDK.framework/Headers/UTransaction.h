...
/*
 mSignia method that allows the SDK to authenticate a user using biotmetric security on the device. The result is captured into the deviceData property from getAuthenticationRequestParameters on MSTransaction
 ...
 */
- (BOOL)close:(NSError *__autoreleasing  _Nullable *_Nullable)error DEPRECATED_MSG_ATTRIBUTE("Use close:spec instead");;
